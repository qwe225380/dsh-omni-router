...
- **Final Convergence（alpha.3）**：OmniTaskState 进入主链；旧多段 prompt 停用；Intervention Gate 冷启动 noop/assist/guard；Evidence per-record trust；DSH Adapter 主链接入；OmniBench 统计修复（false completion / Medium-Hard uplift / per-task paired runs / repo-task gate）。
